...
pub fn open(size: winsize) OpenError!Pty {
    ...
    if (w32.CreatePipe(&self.child_stdin, &self.helper_in, null, 0) == 0) {
        return windows.unexpectedError(windows.kernel32.GetLastError());
    }
    errdefer {
    ...
    if (w32.CreatePipe(&self.wsl_out, &self.child_stdout, null, 0) == 0) {
        return windows.unexpectedError(windows.kernel32.GetLastError());
    }
    errdefer {
    ...
    if (w32.CreatePipe(&self.wsl_err, &self.child_stderr, null, 0) == 0) {
        return windows.unexpectedError(windows.kernel32.GetLastError());
    }
    errdefer {
    ...
    if (w32.CreatePipe(&self.out_pipe, &self.out_write, null, 0) == 0) {
        return windows.unexpectedError(windows.kernel32.GetLastError());
    }
    errdefer {
...
pub fn superviseProcess(self: *Pty, process: windows.HANDLE) void {
    ...
    ) == 0 or w32.AssignProcessToJobObject(job, process) == 0) {
